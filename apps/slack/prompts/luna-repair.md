# Luna bounded mechanical repair

Continue the exact director thread for this create. The trusted worker accepted
your prior artifact envelope, but the host rejected the bundle on a hard
mechanical boundary. The exact rejected canonical bytes are embedded beneath
`inputs/rejected-bundle/`. The host receipt is
...
or another objective execution defect. No taste advisory is included. Fix only
what the evidence proves. Preserve the verified facts and every unrelated
directing choice—concept, story order, copy, palette, typography, layout,
camera, transitions, pacing, and choreography.

Do not call the shell, filesystem, network, MCP, connector, browser, todo-list,
sub-agent, or any other tool. The embedded values are the complete read-only
